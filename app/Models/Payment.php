...
class Payment extends Model
{
    ...
    public function initiatempesa($account, $subscription, $dealer_id = null, $phone)
    {
        ...
        if (!is_null($json)) {
            ...
            return json_encode(['status' => 'error', 'message' => "Error! Ensure you have enough money in you mpesa account then retry."]);
        }
    }
    ...
    public function confirm($checkOutId, $trans_id, $phonenumber, $amount, $completed_at)
    {
        ...
        $plan = ModelsSubscription::where('id', $payment->subscription_id)->first();
        if (!is_null($payment)) {
            $payment->update(['trans_id' => $trans_id, 'amount' => $amount, 'complete' => 1, 'completed_at' => $completed_at]);
            $query = Vehicle::query();
            if (!is_null($payment->dealer_id)) {
                $query->where('dealer_id', $payment->dealer_id);
            }
            $query->orWhere('user_id', $payment->user_id)->update(['priority' => $plan->priority, 'sponsored' => 1]);
        }
        $subscription = new ModelsSubscription();
        $subscription->subscribe($payment);
    }
